...
final class AccountEntry extends ObjectSelectionList.Entry<AccountEntry> {
    ...
    *///?} else {
    ...
    //?}
        ...
        // Glass highlight for hovered/selected/focused rows.
        boolean rowSelected = this.equals(this.list.getSelected());
        boolean rowFocused = this.equals(this.list.getFocused());
        if (rowSelected || rowFocused || hovered) {
            GlassUI.rowHighlight(graphics, x - 4, y - 2, width + 8, height + 4, rowSelected);
        }
        ...
        if (user == null || !this.account.name().equalsIgnoreCase(user.getName())) {
            color = 0xFF_FF_FF_FF;
            dotColor = 0xFF_8A_93_A6; // Grey - not the active session.
        } else if (this.account.uuid().equals(user.getProfileId())) {
            color = 0xFF_00_FF_00;
            dotColor = 0xFF_3D_DC_84; // Green - current logged-in account.
        } else if (this.account.name().equals(user.getName())) {
            color = 0xFF_FF_FF_00;
            dotColor = 0xFF_FF_D5_3D; // Yellow - name matches, UUID doesn't.
        } else {
            color = 0xFF_FF_80_00;
            dotColor = 0xFF_FF_8A_3D; // Orange - partial match.
        }

        // Render a small status dot over the face.
        GlassUI.dot(graphics, x + 6, y + 6, dotColor);
